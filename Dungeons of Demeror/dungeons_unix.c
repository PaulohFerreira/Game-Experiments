...
// FUNÇÕES :: ON
    // FUNÇÕES WINDOWS NO LINUX :: ON
    int getch( ) { // Simula a função getch() no linux
        //*
        ...
        if (enemiesDefeated >= quantEnemies){ // Quando matar todos os inimigos:
            ...
            if(p->posX == DIMENSION_X-1 &&(p->posY == (DIMENSION_Y/2)+1 || p->posY == (DIMENSION_Y/2) || p->posY == (DIMENSION_Y/2)-1)){
                if (actualLevel > 99){// Caso o jogador chegue ao 100º andar, significa que ele ganhou o jogo
                    ...
                    printf("   || How many holes are yet to be created? Only you can say.   ||\n");
                    printf("   ||                                                           ||\n");
    ...
    void updateGame(Char *p){ // Função responsável por atualizar os elementos do jogo
        ...
        system("clear"); // Limpa a tela para ser desenhada de novo
    ...
    void drawHUD(Char *p){ // Função que cria a HUD (Human Interface Display) e a desenha na tela. Também trata da situação de game-over
